...
class ConnectionService:
    ...
    async def _test_snowflake_connection(self, connection: DatabaseConnection) -> ConnectionTestResult:
        """Test Snowflake connection with improved account identifier handling."""
        try:
            ...
            if connection.connection_string:
                ...
                if '.ap-south-1.aws' in account:
                    # Try without region first (recommended)
                    account_only = account.replace('.ap-south-1.aws', '')
                    account_variants = [account_only, account]
                elif '.aws' in account or '.azure' in account or '.gcp' in account:
                    # Other cloud regions
                    parts = account.split('.')
                    account_only = parts[0]
                    account_variants = [account_only, account]
                else:
            ...
            for i, account_variant in enumerate(account_variants):
                try:
                    ...
                    return ConnectionTestResult(
                        status="success", 
                        message=f"Snowflake connection successful using account '{account_variant}'. Version: {version_info[0] if version_info else 'Unknown'}"
                    )
                    
                except Exception as e:
                    last_error = e
                    # If this isn't the last variant, continue trying
                    if i < len(account_variants) - 1:
                        continue
                    else:
                        # This was the last variant, raise the error
                        raise e
            
            # If we get here, all variants failed
            raise last_error or Exception("All account identifier variants failed")
            ...
            if "290404" in error_msg or "404 Not Found" in error_msg:
                # Get account variants for debugging
                account_info = ""
                if connection.connection_string:
                    parsed = urlparse(connection.connection_string)
                    hostname = parsed.hostname
                    ...
                    if '.ap-south-1.aws' in account:
                        account_only = account.replace('.ap-south-1.aws', '')
                        account_info = f" Tried account formats: '{account_only}' and '{account}'"
                
                return ConnectionTestResult(
                    status="error", 
                    message=f"Snowflake account not found. Please verify the account identifier and region.{account_info} Error: {error_msg}"
                )
            elif "390100" in error_msg:
                return ConnectionTestResult(
                    status="error", 
                    message="Invalid username or password for Snowflake."
                )
            elif "390189" in error_msg:
                return ConnectionTestResult(
                    status="error", 
                    message="Snowflake warehouse not found or not accessible."
                )
            elif "250001" in error_msg:
                return ConnectionTestResult(
                    status="error", 
                    message="Snowflake connection timeout. Check network connectivity and account details."
                )
            else:
                return ConnectionTestResult(
                    status="error", 
                    message=f"Snowflake connection failed: {error_msg}"
                )
